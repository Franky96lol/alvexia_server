...
async function armory(username){
    let armory = {};
    for await(let item of global.users[username].armory){
...
async function consumables(username){
    let consumables = {};
    for await(let item of global.users[username].consumables){
...
async function materials(username){
    let materials = {};
    for await(let item of global.users[username].materials){
