...
async function users_save () {
    ...
    for(let user in global.users){
        await fs.writeFile(config.DB + "/accounts/" + user + ".json" , JSON.stringify(global.users[user]) , ()=>{});
    }
    console.timeEnd("Accounts database saved in");
};

async function world_save (){
    console.time("World saved in");
    for(let map in global.world){
        await fs.writeFile(config.DB + "/maps/" + map + ".json" , JSON.stringify(global.world[map]), ()=>{});
    }
